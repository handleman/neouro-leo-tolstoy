...
- Last updated: 2026-09-23. Current: **Phase 2 complete (working tree, uncommitted) → next: Phase 3 naive chat**.
...
- **Phase 2 — Baseline retrieval: DONE, uncommitted** (journal `notebooks/phase-2-journal.md`; plan `docs/design-docs/0008-phase-2-baseline-retrieval.md`). Evidence: 26,188 chunks in local `.chroma/` (`tolstoy-ru`), `POST /search` + REPL live, 38 tests green, RU/EN seed queries retrieve the same RU chunks. To commit: `tolstoy/ingest/chunk.py`, `tolstoy/store/`, `tolstoy/index.py`, `tolstoy/search.py`, API upgrade, `test_chunk.py`, `test_store.py`, config + `.env.example` (`TOLSTOY_CHUNK_MIN_CHARS`).
